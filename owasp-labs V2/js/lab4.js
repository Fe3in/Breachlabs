...
const LAB4_STEPS = [
  ...
  },
  {
    ...
    simHtml: `
      <div class="fake-app">
        <div class="fake-topbar">POST shopfast.io/reset-password</div>
        <div class="fake-body">
          <div class="answer-row" style="margin-bottom:0;">
...
const LAB4_INTRO = {
  summary:
    "Insecure Design flaws are baked into the architecture itself, before a single line of code is written. This lab shows what happens when a password-reset flow was designed around a predictable, sequential token instead of a cryptographically random one — no coding bug required to break it.",
  vulnerableCode:
`let nextToken = 100000;
function issueResetToken() {
  return \`rst-\${nextToken++}\`; // predictable!
}`,
  secureCode:
...
const LAB4_REMEDIATION = [
  'Generate reset tokens with a cryptographically secure random generator, never a counter.',
  'Expire tokens quickly (e.g. 15 minutes) and invalidate them immediately after first use.',
  'Rate-limit the reset endpoint to block brute-force guessing.',
  'Bring security requirements into design and threat-modeling before implementation, not after.',
];
