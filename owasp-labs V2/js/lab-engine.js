...
const LabEngine = (() => {
  ...
  function renderIntro() {
    const el = document.getElementById('lab-intro');
    if (!el || !intro) return;
    el.innerHTML = `
      <div class="intro-panel">
        <div class="intro-label">Vulnerability overview</div>
        <p class="intro-summary">${intro.summary}</p>
        ${intro.vulnerableCode && intro.secureCode ? `
          <div class="code-compare">
            <div class="code-block bad">
              <div class="code-block-label">✗ Vulnerable</div>
              <pre>${escapeHtml(intro.vulnerableCode)}</pre>
            </div>
            <div class="code-block good">
              <div class="code-block-label">✓ Secure</div>
              <pre>${escapeHtml(intro.secureCode)}</pre>
            </div>
          </div>` : ''}
      </div>`;
  }
  ...
  function renderCompletionBanner() {
    ...
    const remRoot = document.getElementById('remediation-root');
    if (remRoot && progress.completed && remediation && remediation.length) {
      remRoot.innerHTML = `
        <div class="remediation-box">
          <div class="remediation-label">How to fix this</div>
          <ul>${remediation.map((r) => `<li>${r}</li>`).join('')}</ul>
        </div>`;
    }
  }
  ...
  return {
    async init(config) {
      ...
      remediation = config.remediation || null;
