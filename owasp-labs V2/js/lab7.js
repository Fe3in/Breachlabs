...
const LAB7_STEPS = [
  {
    ...
    simHtml: `
      <div class="fake-app">
        <div class="fake-topbar">POST shopfast.io/api/register</div>
        <div class="fake-body">
...
const LAB7_INTRO = {
  summary:
    "Authentication Failures cover weaknesses in verifying identity: weak password policies, missing rate limiting or lockout on login, and sessions that aren't properly managed — like a session ID that's never regenerated after login (session fixation).",
  vulnerableCode:
`// no password policy enforced
if (password.length >= 1) createAccount(username, password);

// session ID never regenerated after login
req.session.id = req.cookies.sessionid || generateNew();`,
  secureCode:
`// enforce a real password policy + rate-limit failed logins
...
const LAB7_REMEDIATION = [
  ...
  'Always regenerate the session ID on login to prevent session fixation.',
  'Use short-lived, rotated session tokens with secure cookie flags (HttpOnly, Secure, SameSite).',
];
