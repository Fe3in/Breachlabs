...
const LAB1_STEPS = [
  {
    ...
    desc: 'ShopFast Pay\'s mobile API exposes a debug endpoint that echoes your session. Call it and note your account ID — you\'ll need it in a moment.',
    simHtml: `
      <div class="fake-app">
        <div class="fake-topbar">GET shopfast.io/pay/api/session</div>
        <div class="fake-body">
  ...
  },
  {
    ...
    simHtml: `
      <div class="fake-app">
        <div class="fake-topbar">GET shopfast.io/pay/api/account/:id/balance</div>
        <div class="fake-body">
  ...
  },
  {
    ...
    simHtml: `
      <div class="fake-app">
        <div class="fake-topbar">GET shopfast.io/pay/api/admin/panel</div>
        <div class="fake-body">
          <div class="answer-row" style="margin-bottom:0;">
...
const LAB1_INTRO = {
  summary:
    "Broken Access Control happens when an application fails to properly enforce what an authenticated user is actually allowed to do. Attackers exploit this by directly modifying identifiers or parameters — account IDs, roles, resource paths — to reach data or actions outside their own permissions. It's consistently the most common issue in real-world web apps.",
  vulnerableCode:
`app.get('/api/account/:id/balance', (req, res) => {
  const account = db.getAccount(req.params.id);
  res.json(account); // no ownership check!
});`,
  secureCode:
`app.get('/api/account/:id/balance', requireAuth, (req, res) => {
  const account = db.getAccount(req.params.id);
...
const LAB1_REMEDIATION = [
  'Enforce object-level authorization on every request — never trust a client-supplied ID alone.',
  'Deny by default; require an explicit ownership or role check before returning data.',
  'Never trust a "role" value sent by the client — always re-verify permissions server-side.',
  'Write automated tests for access-control edge cases (both horizontal and vertical privilege escalation).',
];
