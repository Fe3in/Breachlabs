...
const LAB2_STEPS = [
  {
    ...
    simHtml: `
      <div class="fake-app">
        <div class="fake-topbar">POST shopfast.io/pay/login  (captured, plaintext HTTP)</div>
        <div class="fake-body">
...
const LAB2_INTRO = {
  summary:
    "Cryptographic Failures happen when sensitive data isn't properly protected at rest or in transit — using encoding instead of real encryption, hashing passwords without a salt (or with a fast, crackable algorithm), or shipping secrets hardcoded in source. The result is data that looks 'protected' but isn't.",
  vulnerableCode:
`// "encrypting" a password before sending it
const encoded = btoa(password); // base64 is NOT encryption

// storing it
const hash = sha256(password); // no salt, fast — crackable`,
  secureCode:
`// Always use TLS in transit, never encoding as "encryption"
const hash = await bcrypt.hash(password, 12); // slow, salted, tunable`,
};

const LAB2_REMEDIATION = [
  'Never use encoding (base64, hex) as a substitute for real encryption.',
  'Hash passwords with a slow, salted algorithm like bcrypt or argon2 — never raw SHA-256/MD5.',
  'Enforce TLS everywhere; never transmit credentials over plain HTTP.',
  'Keep secrets out of source code — use environment variables or a secret manager, and scan builds for leaked keys.',
];
